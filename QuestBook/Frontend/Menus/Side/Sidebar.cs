...
public class Sidebar : IMenu
{
    ...
    private void AlignButtons(List<ButtonInfo> buttonInfos, ContentManager content, TextureAtlas atlas)
    {
        ...
        int height = (int)((Destination.Height - (2 * borderHeight)) / buttonInfos.Count * 0.75);
        int spacing = (int)((Destination.Height - (2 * borderHeight)) / buttonInfos.Count * 0.25);
        int width = (int)(Destination.Width * 0.65f);
        
        Point start = new Point((int)(Destination.X + ((Destination.Width / 2) - (width / 2))), (int)(Destination.Y + borderHeight));
